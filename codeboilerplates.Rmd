...
indexdf <- read.xlsx("Data set 3 Oct- Gaming the peer review system.xlsx",sheetIndex=1)
...
removesolo<-1
if(removesolo==1){
  solorevs<-c('Lopez A','Lucchese D','Signore F','Sorrentino F','Riemma G','Serra P','Insigna S','Ganduscio S','Lumia V','Insinga S') #first mention of Insinga does't seem to pick him up! 
...
coded_df <- read.xlsx('allrevs_031025.xlsx',sheetName="Reviews")
boilercode<-read.xlsx('allrevs_031025.xlsx',sheetName="boiler10")
...
Estimate how many additional citations the authors obtained

```{r coerced}
dfmp$Count
dfmp$benefit<-dfmp$isauthor*dfmp$Count.old
table(dfmp$Reviewer,dfmp$benefit)
```
               
Supplement 2 will have subset of columns of coded_df
```{r createSupp2}
supp2<-coded_df[,c(9,1,2,3,8,10,13,11)]
supp2$reviewer<-NA
...
write.xlsx(supp2,"Supplement2.xlsx")
```
