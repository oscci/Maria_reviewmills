...
indexdf <- read.xlsx("Data set 5 Oct- Gaming the peer review system.xlsx",sheetIndex=1)
...
wantcols <- c('ID','REVIEWER','REVIEWED.ARTICLE.JOURNAL','REVIEWED.ARTICLE.PUBLISHER','REVIEWED.ARTICLE.DOI',
              'Stock.phrases.included','PMIDs.mentioned','PMID.in.final.version.refs','coerced..9.for.resisted.','comparison.rev','Nrev.in.target.set','revmilled')
indexdf<-indexdf[,wantcols]
names(indexdf)<-c('ID','Reviewer','Journal','Publisher','DOI','Boilerplate','PMID','cited', 'coerced','comparison.rev','NrevThisArticle','revmill.ed')
#remove blank row
...
coded_df <- read.xlsx('allrevs_051025.xlsx',sheetName="Reviews")
boilercode<-read.xlsx('allrevs_051025.xlsx',sheetName="boiler10")
...
con_df<-read.xlsx("Comparison data/all_control_revs_coded_05Oct2025.xlsx",sheetName="Reviews")
...
temp<-coded_df[1:nrow(con_df),]
temp[,3:ncol(temp)]<-NA  #dummy to hold data
temp[,1:2]<-con_df[,1:2]
wantcols<-c('N','final','text')
w<-which(names(con_df) %in% wantcols)
ww<-which(names(coded_df) %in% wantcols)
for (i in 1:length(w)){ #need to just read in relevant columns
  temp[,ww]<-con_df[,w]
  }
...
pmids <- read.xlsx('pmids/PMIDsfull_4_Oct.xlsx',sheetIndex=1)
pmids <-pmids[pmids$includes.revmiller==1,]
...
```{r createSupp2}
supp2<-coded_df[,c(1,9,2,3,8,10,13,11)]
supp2$reviewer<-NA
for (i in 1:(length(revnames)-1)){
  w<-which(supp2$Reviewer==revnames[i])
  supp2$reviewer[w]<-paste0('Reviewer ',i)
}
w<-which(supp2$Reviewer=="X_Anon")
supp2$reviewer[w]<-'Anon'
...
```
NB Supplement 1 is a tidied up version of "Data set 3 Oct- Gaming the peer review system.xlsx"
...
#This would be a real pain to coded, so done by looking at table(coded_df$citationtype)

N7 = 9+5+10+1
w<-which(coded_df$citationtype %in% c('P1C1','P1D1','P2','T2'))
N8 <- length(w)

N9 <- nrow(pmids) #those not by review miller excluded after reading PMIDS
